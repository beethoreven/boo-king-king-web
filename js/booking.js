...
export function createBookingView() {
  ...
  async function submit() {
    ...
    const date = currentDate();
    const time = currentTime();
    // ★ 這裡不講金額。這家店不收訂金（functions 表 deposit=off），而售價
    //   是到場才結的，寫在上方的標籤列就夠——在「確認預約」這一步再報一次
    //   價，會讓人以為按下去要先付錢。
  ...
  function render() {
    ...
    for (const tag of [
      ...
      // 沒有訂金那一格：這家店不收訂金（functions 表 deposit=off）。
...
function slotMessage(slot) {
  // 店家忙碌日排在最最前面：它比排期的每一層都高（案主 2026-09-17）。
  // ★ 不併進下面那句「此時間未開放」：那句會讓人去試別的時間，但忙碌日
  //   那天不管幾點都不行，要他換的是日期。後端單獨回 store_busy 就是為了這句。
  if (slot.store_busy) return '店家這天不開放預約任何劇本，請選擇其他日期。';
