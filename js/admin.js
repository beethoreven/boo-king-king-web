...
export function createAdminView({ tab, sub } = {}) {
  ...
  // 過期的回應不要蓋掉比較新的結果。★ 這一段是正本，其他畫面指過來。
  //
  ...
  // （最早的寫法與一個實測到的症狀見 calendar.js 的 seq。）
  ...
  function renderBookingsList() {
    ...
    const nodes = [
      ...
      el('div', { class: 'section toolbar' }, [
        el('button', {
          class: 'btn btn--ghost btn--small',
          // 收起 = 不篩了。★ 這一段是正本，其他畫面指過來。
          //
          // 「收起篩選器」在使用者眼中就是「不篩了」。原本只是把面板藏
          // 起來、條件照舊，而清除按鈕在面板裡——藏起來之後就沒有任何路
          // 可以回到完整清單，只能重新整理。那是死路，不是設計。
          //
 ...
 *
 * ★ 這家店的版本沒有主持人區、也沒有訂金欄位（single 模式、不收訂金），
 *   所以存檔前也不問「主持人尚未確認」。差異在這支函式裡，不在呼叫端——
 *   場次管理與場次行事曆拿到的是同一張。
 */
export function bookingEditorNode(ctx) {
  const { b, scripts, users } = ctx;
